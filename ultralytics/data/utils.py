...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            ...
            LOGGER.info("")
            m = f"datasets '{name}' images not found, missing path '{next(x for x in val if not x.exists())}'"
            ...
            LOGGER.info(f"datasets download {s}\n")
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not data_dir.is_dir():
        ...
        LOGGER.info("")
        LOGGER.warning(f"datasets not found, missing path {data_dir}, attempting download...")
        t = time.time()
        ...
        LOGGER.info(f"datasets download success ✅ ({time.time() - t:.1f}s), saved to {colorstr('bold', data_dir)}\n")
    train_set = data_dir / "train"
    if not train_set.is_dir():
        LOGGER.warning(f"datasets 'split=train' not found at {train_set}")
    ...
    if split == "val" and not val_set:
        LOGGER.warning("datasets 'split=val' not found, using 'split=test' instead.")
        val_set = test_set
    elif split == "test" and not test_set:
        LOGGER.warning("datasets 'split=test' not found, using 'split=val' instead.")
        test_set = val_set
...
class HUBDatasetStats:
    ...
    Args:
        ...
        task (str): datasets task. Options are 'detect', 'segment', 'pose', 'classify'.
        autodownload (bool): Attempt to download dataset if not found locally.

    Attributes:
        task (str): datasets task type.
        ...
        data (dict): datasets configuration data.
