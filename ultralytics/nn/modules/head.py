...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect", "DecoupledDetect"
...
class DecoupledHead(nn.Module):
    ...
    def __init__(self, ch=256, nc=80, anchors=()):
        ...
        self.merge = nn.Conv2d(ch, 256, 1, 1, 0)
...
class DecoupledDetect(nn.Module):
    """YOLOv8 风格解耦检测层 (anchor-free)"""

    def __init__(self, nc: int = 80, ch: tuple = (), reg_max: int = 16, end2end: bool = False):
        """
        Args:
            nc (int): number of classes
            ch (tuple): channels of input feature maps (from backbone/head)
            reg_max (int): DFL channels
            end2end (bool): whether to use end2end one2one layers
        """
        super().__init__()
        self.nc = nc
        self.nl = len(ch)           # number of detection layers
        self.reg_max = reg_max      # DFL channels
        self.no = nc + 4 * reg_max  # xywh + reg_max*4 + classes
        ...
        # one2one layers for end2end training
        if self.end2end:
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)

        # strides (later assigned during model build)
        self.stride = torch.zeros(self.nl)

    def forward(self, x: list[torch.Tensor]):
        # 训练模式：返回 raw feature map，用于 loss
        if self.end2end:
            return self.forward_end2end(x)
        ...
        if self.training:
            return x  # 保留 raw output

        # 推理模式：sigmoid + reshape
        z = []
        for i in range(self.nl):
            bs, _, ny, nx = x[i].shape
            y = x[i].sigmoid()
            y = y.permute(0, 2, 3, 1).reshape(bs, -1, self.no)
            z.append(y)

        return torch.cat(z, 1) if not self.export else (torch.cat(z, 1), x)
