...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            A2C2f,
            # --- 新增模块 ---
            Faster_C3k2,  # Added: PConv based C3k2
            Faster_C3k2_SimAM,  # Added: Faster + SimAM
            C3k2_Sc,  # Added: ScConv based C3k2
            SPDConv,  # Added: No-loss Downsampling
            ScConv,  # Added: Spatial/Channel Reconstruction Conv
        }
    )

    # ==============================================================================
    # 2. 注册重复模块 (Repeat Modules)
    # 这些模块支持 'n' 参数 (堆叠次数)
    # ==============================================================================
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # --- 新增模块的特殊参数处理 ---
            if m is Faster_C3k2 or m is Faster_C3k2_SimAM or m is C3k2_Sc:
                # 类似 C3k2 的逻辑，如果需要针对不同 scale 调整参数可以在这里加
                # 目前保持通用即可
                pass
            ...
            c2 = ch[f]  # ECA 不改变通道数
            args = [ch[f], *args]  # 自动注入 c1

        # ==============================================================================
        # 3. 新增特殊模块的处理 (Special Handlers)
        # 处理参数签名不符合 (c1, c2) 规范的模块
        # ==============================================================================
        elif m is DySample:  # Dynamic Upsampling
            c1 = ch[f]
            c2 = c1  # DySample 通常保持通道数不变，或由 scale 决定，这里假设不变
            args = [c1, *args]  # 注入 in_channels

        elif m is EMA:  # Efficient Multi-Scale Attention
            c1 = ch[f]
            c2 = c1  # Attention 不改变通道数
            args = [c1, *args]  # 注入 channels 参数
        ...
        elif m in frozenset({Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn,
                             v10Detect, DecoupledDetect}):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, DecoupledDetect}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, DecoupledDetect)):
                return "detect"
