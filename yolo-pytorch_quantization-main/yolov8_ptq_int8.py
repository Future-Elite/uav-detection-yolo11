...
def export_quant_params(model, output_dir="quant_params"):
    import json
    import os
    ...
    for name, module in model.named_modules():
        ...
        if hasattr(module, "_input_quantizer"):
            ...
            if input_quantizer._amax is not None:
                layer_params["input_scale"] = input_quantizer._amax.item() / 127.0  # scale = amax / (2^b-1)
                layer_params["input_zero_point"] = 0  # 对称量化的zero point通常为0
        
        # 提取权重量化参数
        if hasattr(module, "_weight_quantizer"):
            weight_quantizer = module._weight_quantizer
            if weight_quantizer._amax is not None:
                layer_params["weight_scale"] = weight_quantizer._amax.item() / 127.0
                layer_params["weight_zero_point"] = 0
        
        # 提取激活量化参数（假设激活使用相同的量化器）
        if hasattr(module, "activation_quantizer"):
            activation_quantizer = module.activation_quantizer
            if activation_quantizer._amax is not None:
                layer_params["activation_scale"] = activation_quantizer._amax.item() / 127.0
                layer_params["activation_zero_point"] = 0
        
        if layer_params:
            quant_params[name] = layer_params
    
    # 保存为JSON文件
    output_path = os.path.join(output_dir, "quant_params.json")
    with open(output_path, "w") as f:
        json.dump(quant_params, f, indent=4)
    print(f"Quantization parameters saved to {output_path}")
def prepare_model(calibrator, opt, device):
    ...
    quant.initialize_calib_method(per_channel_quantization=False, calib_method=calibrator)  
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='/media/8T3/rh_xu/yolov11/thermal-1/data.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default="/media/8T3/rh_xu/yolov11/runs/detect/train/weights/best.pt", help='model.pt path(s)')
    parser.add_argument('--model-name', '-m', default='yolov11n', help='model name: default yolov11n')
    ...
    parser.add_argument('--sensitive-layer', default=None, help="skip sensitive layer: default detect head, e.g. ['model.15.cv1.conv',\
                                                      'model.15.cv2.conv',\
                                                      'model.15.m.0.cv1.conv',\
                                                      'model.15.m.0.cv2.conv']")
    ...
    parser.add_argument('--out-dir', '-o', default='/media/8T3/rh_xu/yolov11/yolo-pytorch_quantization/quantized_weight', help='output folder: default ./runs/finetune')
...
if __name__ == "__main__":
    ...
    export_quant_params(model, output_dir=opt.out_dir)
    print(model,file=open('/media/8T3/rh_xu/yolov11/yolo-pytorch_quantization/yolo-pytorch_quantization/yolov11n_ptq_detect_model.txt','w'))
    
    onnx_filename = '/media/8T3/rh_xu/yolov11/yolo-pytorch_quantization/yolo-pytorch_quantization/yolov11n_ptq_detect.onnx'
